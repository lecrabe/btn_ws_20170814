...
rootdir <- "/media/dannunzio/OSDisk/Users/dannunzio/Documents/countries/bhutan/aa_change_951016/"
setwd(rootdir)
rootdir <- paste0(getwd(),"/")
imgdir  <- "/media/dannunzio/hdd_remi/bhutan/"
...
lsat_dir <- paste0(imgdir,"time_series_image_dir/landsat/")

#### Name of the directory where your Sentinel data is
stnl_dir <- paste0(imgdir,"time_series_image_dir/sentinel/")

#### Name of the directory where your data will be stored in output
dest_dir <- paste0(imgdir,"clip_time_series_revision/")

#### Name of the directory where your map
maps_dir <- paste0(imgdir)
the_map  <- raster(paste0(maps_dir,"DD_2004_2009_druk_20170907.tif"))
...
## Read the datafile 
pts <- read.csv("/media/dannunzio/OSDisk/Users/dannunzio/Documents/countries/bhutan/aa_change_951016/check_commission_loss.csv")  #####  CHANGE TO MY VALUE HERE
head(pts)
names(pts)
...
list_lsat <- list.files(lsat_dir,pattern=glob2rx(paste0("*lsat_",yr_str_lsat,"*")))
lp <- list()
...
list_s2 <- list.files(stnl_dir,pattern=glob2rx(paste0("*s2_",yr_str_stnl,"*")))
lp<-list()
