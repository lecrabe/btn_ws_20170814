...
pts_results <- read.csv("AA_bhutan_work/res_all_2004_2009.csv")
  pts_origin  <- read.csv("AA_bhutan_work/CEP/CE_2017-09-11_2004_2009.csv")
...
out <- pts_origin[
  pts_origin$id %in% pts_results[pts_results$ref_class == 2 & 
                                 pts_results$map_class != 2,]$id,
  ]

## Export as csv file
write.csv(out,paste("check_commission_loss.csv",sep=""),row.names=F)
